...
## Note
- Running the code you will also get quite a lot of information as terminal output. That information is all relevant to my course work.
  The reason it is in the last "if" statement is because python would throw errors otherwise. This code is published so I can keep track of it and pull it if I need to make any changes when submitting my work.
  As such it has high specificity to the my specific circuit analysis problem. It can be reused, as long as the circuit is of the same type (meaning it has the same amount of elements and they are arranged in the same way), and your initial       signal is a square wave. You may have to adjust the Constant values at the start of the file if your problem has different values.
  You can adjust the range of the for-loop, which will provide you with a graphic of a fourier approximation of the voltage source $e_{A}(t)$ with more harmonics. More harmonics means a better approximation, and it's pretty cool to see at k = 50, or even k = 100, how close we can get to an almost ideal square wave.
